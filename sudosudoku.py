...
#This stack-based sudoku solver was inspired by Project Euler problem 96 (https://projecteuler.net/problem=96)
#Its main method, 'solve(sudokufilename)', takes a .txt file containing sudokus as
#input and creates a .txt file 'solved_sudokufilename' of the solved puzzles.
...
#This method reads multiple sudokus from the file 'sudokufilename', which must
#be in the same format as the file given by Project Euler (sudoku.txt)
...
def possMatrix(s):
    ...
    for i in range(0,9):
        ...
        for j in range(0,9):
            ...
            if s[i][j]==0:
                for n in range(1,10):
                    if not (inRow(i,j,n,s) or inCol(i,j,n,s) or inBox(i,j,n,s)):
                        p.append(n)
            row.append(p)
        poss.append(row)
    return poss
...
#The following methods determine whether the number n in sudoku s is a possible
#entry for a (distinct) entry in the same row, column, or box as the entry i,j
...
#This determines whether there is an unsatisfiable entry in s based on its poss
def broken(s,poss):
    for i in range(0,9):
        for j in range(0,9):
...
#
#            if the sudoku is impossible ("LOGIC IS BROKEN" condition), then pop from the stack and discard it.
#            
#        if the sudoku is solved, return it!
#        if it isnt solved, but no progress is being made, look for an entry with a small possibilities list, push the sudokus with all of those guesses onto the stack 
def SOLVESU(s):
    stack=[]
    stack.append(s)
    ...
    while not solved(cur):
        ...
        prog=True #this ensures progress is being made on logically solving s
        while prog==True and not broken(cur,poss):
            prog=False
            for i in range(0,9):
                for j in range(0,9):
                    if cur[i][j]==0 and prog==False:
                        if len(poss[i][j])==1:
                            cur[i][j]=poss[i][j][0]
                            poss=possMatrix(cur)
                            prog=True
                            ...
                            if len(poss[i][j])>1:
                                for n in poss[i][j]:
                                    if not inPossRow(i,j,n,poss) or not inPossCol(i,j,n,poss) or not inPossBox(i,j,n,poss):
                                        cur[i][j]=n
                                        poss=possMatrix(cur)
                                        prog=True
        ...
        if not solved(cur) and not broken(cur,poss):
            ...
            choice=False
            l=1 #the number of choices for the guess that must be considered
    ...
    for s in sus:
        ...
        for n in range(0,9):
            for m in range(0,9):
                if x[n][m]==0:
                    solved=0
        #The problem asks for the sum of the first 3 digits, considered as a single number, from each sudoku.
